...
== link:CopyPasteToApp.ahk[]

This script copies the current selection into the clipboard, switches to a given target window (e.g. editor), pasts the clipboard content and returns back to the previous window.

[cols="1,3",options="header"]
|===
|Key|Function
|`Win-Alt-e`|Copy (text only) to Visual Studio Code, before pasting press `End`, after pasting press `Return, Home`.
|===
...
----
;; copy to Notepad++
#!e::_copyPasteToApp("ahk_class i)^Notepad\+\+$")

;; copy to emacs using Ctrl-y to paste
#!e::_copyPasteToApp("ahk_class i)^Emacs$",, "^y")
...
;; Copy to WordPad into a new line and add newline after pasting, copy non-text (e.g. images) too
