...
All included scripts expect that the following parametrs are set:
...
All AHK scripts listed below are included in this core script. In addition at least the following key-bings are defined:
...
Some basice key(re-)maps. Note that my default keyboard layout is https://colemak.com[Colemak]:
...
|====
|Active|Key|Action|Description
|{icon-ok}|`XButton1` _(Mouse 4)_|`PageUp`|Faster paging than mouse-wheel, if needed. Replaces prev/next (usually useless, key-combo).
|{icon-ok}|`XButton2` _(Mouse 5)_|`PageDown`|_See above ..._
|{icon-ok}|`Win-Alt-Space`|`MediaKey: Pause`|Easy acces, esp. if dedicated media keys are not availlable.
...
|{icon-ok}|`Alt/Shift-ScrollLock`|Toggle CapsLock|Noone really needs CapsLock nowadays but sometimes it is activated for no apparent reason, so it's good to have a key-binding to deactivate it._
...
|{icon-x}|`[/]`|`(/)`|Classic lisp hacker / emacs mapping, but also practical foro most users in daily life (since `(/)` more commonly used, but dedicated keys for less used `[/]`. Today I favor the mappings shown below.
|{icon-ok}|`AltGr-n/e`|`(/)`|Homerow access _(`j/k` on US layout)_.
|{icon-ok}|`AltGr-N/E`|`[/]`|Homerow access _(`J/K` on US layout)_.
|{icon-ok}|`AltGr-i/o`|`{/}`|Homerow access _(`l/;` on US layout)_.
|====
