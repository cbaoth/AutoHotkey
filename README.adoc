...
This scripts confines the mouse to a window's screen area, either of the window currently below the mouse cursor _(default)_ or the active window. The confinement area can optionally be reduced by a fixed margint to exclude the window decoration.

Mouse cursor confinement is sometimes desired when working in a mouse-heavy application or games _(e.g. in windowed mode)_, especially in a mutli-screen environment. The scripts helps avoiding exiting the working area and prevent unwanted clickes _(e.g. taskbar, different app, etc.)_.
